...
const findEvery = (arr, target) => {
  // Solution code here...
  return !arr.map(value => value.includes(target)).includes(false);
};
...
const unenrollBrook = (arr) => {
  ...
  return arr.reduce((acc,curr)=>{    
    acc.push(curr.filter(value => !value.includes("Brook")));
    return acc;
  },[])
};
/* ------------------------------------------------------------------------------------------------
...
const sortByDay = (arr) => {
  // Solution code here...
  let result = [];
  for(let i=0;i<daysOfWeek.length;i++){  result[i] = [];  }

  daysOfWeek.forEach((target,index) => {
    arr.map(value =>{
