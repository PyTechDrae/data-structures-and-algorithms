...
const sortByChildren = (charArray) => {
  ...
  charArray.sort((a,b) =>
  {
    let numChildrenA = a.children  ? a.children.length : 0;
    let numChildrenB = a.children  ? b.children.length : 0;
    let houseNameA = a.name;
    let houseNameB = b.name;
...
const hasChildrenEntries = (arr, character) => {
  // Solution code here...
  for(let i in arr){
    let entry = Object.entries(arr);
    for(let j in entry){
      if(entry[j][1].name === character){
        return entry[j][1].children.length > 0;
      }
    }
  }
  return false;
...
const totalCharacters = (arr) => {
  ...
  console.log(arr);
  for(let i in arr){
    let man = arr[i].name !== null ? 1 : 0;
    let spouse = arr[i].spouse !== null ? 1 : 0;
    let numChildren = arr[i].children !== null ? arr[i].children.length : 0;
    result += man;
    result += spouse;
    result += numChildren;
  }
...
const houseSize = (arr) => {
  ...
  for(let i in arr){
    ...
    let spouse = arr[i].spouse !== null ? 1 : 0;
    let numChildren = arr[i].children !== null ? arr[i].children.length : 0;
    let houseCount = man + spouse + numChildren;
    let home = {'house': arr[i].house,'members':houseCount};
    sizes.push(home);
  }
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  for(let i in arr){
    let man = arr[i].name !== null ? 1 : 0;
    let spouse = !deceasedSpouses.includes(arr[i].spouse) && arr[i].spouse !== null ? 1 : 0;
    let numChildren = arr[i].children !== null ? arr[i].children.length : 0;
    let houseCount = man + spouse + numChildren;
    let home = {'house': arr[i].house,'members':houseCount};
    survivors.push(home);
